...
export function useTokenList({
  ...
  const buildFilters = useCallback((): TokenSearchFilters => {
    ...
    if (activeFilter) {
      ...
      if (activeFilter.bcurveMin !== undefined) f.bcurveMin = activeFilter.bcurveMin;
      if (activeFilter.bcurveMax !== undefined) f.bcurveMax = activeFilter.bcurveMax;
      if (activeFilter.holdersMin !== undefined) f.holdersMin = activeFilter.holdersMin;
      if (activeFilter.holdersMax !== undefined) f.holdersMax = activeFilter.holdersMax;
      if (activeFilter.coinAgeMin !== undefined) f.coinAgeMin = activeFilter.coinAgeMin;
      if (activeFilter.coinAgeMax !== undefined) f.coinAgeMax = activeFilter.coinAgeMax;
      if (activeFilter.onlyShow?.length) f.onlyShow = activeFilter.onlyShow;
      if (activeFilter.doNotShow?.length) f.doNotShow = activeFilter.doNotShow;
    }
    return f;
  ...
  const fetchFirst = useCallback(async () => {
    ...
    try {
      ...
      // Fallback: use existing data, or mock data if nothing is available
      setTokens((prev) => {
  ...
  }, [buildFilters, searchQuery, tokens?.data?.length]);
