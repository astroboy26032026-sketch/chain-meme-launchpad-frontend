...
========================= */
type CosmicTier = 'SEED' | 'SPROUT' | 'SAPLING' | 'TREE' | 'BIG_TREE';

const TIER_COSMIC_BY_NUM: Record<number, CosmicTier> = {
  1: 'SEED',
  2: 'SPROUT',
  3: 'SAPLING',
  4: 'TREE',
...
const TIER_EMOJI: Record<CosmicTier, string> = {
  SEED: '☄️',
  SPROUT: '🌙',
  SAPLING: '🪐',
  TREE: '⭐',
...
const TIER_LABEL: Record<CosmicTier, string> = {
  SEED: 'Comet',
  SPROUT: 'Moon',
  SAPLING: 'Planet',
  TREE: 'Star',
...
const TierBadge: React.FC<{ tierNum: number }> = ({ tierNum }) => {
  const plant = TIER_COSMIC_BY_NUM[tierNum] ?? 'SEED';
  return (
...
type HistoryCategory = 'ALL' | 'TRADE' | 'REFERRAL' | 'DAILY';

const CATEGORY_TABS: { key: HistoryCategory; emoji: string; label: string }[] = [
  { key: 'ALL',      emoji: '🧭', label: 'ALL' },
  { key: 'TRADE',    emoji: '⚡', label: 'TRADING' },
  { key: 'REFERRAL', emoji: '🤝', label: 'REFERRAL' },
  { key: 'DAILY',    emoji: '✨', label: 'DAILY' },
];
...
const PointsPage: React.FC = () => {
  ...
  const nextTierLabel = useMemo(() => {
    ...
    const plant = TIER_COSMIC_BY_NUM[tierProg.nextTier.tier] ?? 'SEED';
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (category === 'TRADE') return t.includes('TRADE');
      if (category === 'REFERRAL') return t.includes('REFERRAL');
      if (category === 'DAILY') return t.includes('DAILY') || t.includes('LOGIN');
      return true;
    });
  }, [rows, category]);
  ...
  const plant = TIER_COSMIC_BY_NUM[tierProg?.currentTier.tier ?? 1] ?? 'SEED';
    ...
    <Layout>
      ...
      <div className="min-h-screen flex flex-col items-center py-8 sm:py-10">
        <div className="w-full max-w-5xl px-4 sm:px-6">
          ...
          {/* ── Card 3: Spin to Win ── */}
          <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card)] flex-1 flex flex-col">
            <div className="px-5 pt-4 pb-1">
            ...
            <div className="px-5 pb-5 flex-1 flex flex-col">
              ...
              <div className="mt-4 rounded-xl p-3 border border-[var(--card-border)] flex-1"
          ...
          <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card)] overflow-hidden flex flex-col">
            <div className="px-5 pt-4 pb-0">
              <span className="text-[10px] font-extrabold tracking-[0.18em] uppercase opacity-60">{POINTS.POINT_HISTORY}</span>
            </div>

            {/* Category tabs */}
            <div className="flex items-end gap-0 border-b border-[var(--card-border)] px-2 mt-2">
              {CATEGORY_TABS.map(({ key, emoji, label }) => {
                const isActive = key === category;
                return (
                  <button
                    key={key}
                    onClick={() => { setCategory(key); setVisibleCount(HISTORY_STEP); }}
                    className={`
