...
import { fmtSol } from '@/utils/formatters';
import {
  claimReward,
  getRewardInfo,
  getRewardMarquee,
...
/* StatusModal removed — using system toasts (toastSuccess/toastError) instead */
...
const WHEEL_SEGMENTS = [
  { label: '0.2 SOL',  value: 0.2,  color: '#A78BFA', textColor: '#1e1040', emoji: '🌟' },
  { label: '1 SOL',    value: 1,    color: '#60A5FA', textColor: '#0c1a3a', emoji: '🪐' },
  { label: '0.5 SOL',  value: 0.5,  color: '#F472B6', textColor: '#3b0520', emoji: '⭐' },
  { label: '5 SOL',    value: 5,    color: '#34D399', textColor: '#022c22', emoji: '🚀' },
  { label: '0.2 SOL',  value: 0.2,  color: '#FB923C', textColor: '#3b1106', emoji: '✨' },
  { label: '10 SOL',   value: 10,   color: '#818CF8', textColor: '#1e1040', emoji: '🌌' },
  { label: '0.5 SOL',  value: 0.5,  color: '#F87171', textColor: '#3b0808', emoji: '🛸' },
  { label: '50 SOL',   value: 50,   color: '#FBBF24', textColor: '#3b2506', emoji: '💫' },
  { label: '1 SOL',    value: 1,    color: '#2DD4BF', textColor: '#022c22', emoji: '🌙' },
  { label: '0.2 SOL',  value: 0.2,  color: '#E879F9', textColor: '#3b0540', emoji: '☄️' },
];
...
const RewardPage: React.FC = () => {
  ...
  const openStatusModal = (title: string, text: string, tone: 'success' | 'error') => {
    if (tone === 'error') toastError(text, title);
    else toastSuccess(text, title);
  };
  ...
  return (
    <Layout>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-10 pt-6 relative overflow-hidden">
        <div
              ...
              return (
                <span
                  ...
                  className="mx-3 inline-flex items-center gap-2 px-4 py-1.5 rounded-full
                             bg-[var(--card2)]/75 text-[15px] font-semibold leading-snug"
                >
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card)] p-4 text-center">
                ...
                <div className="text-xl font-extrabold text-[var(--primary)] mb-1">{points}</div>
                <p className="text-[10px] text-gray-500">Earn from quests & trading</p>
              </div>
            </div>

            {/* Prize Table */}
            <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card)] p-4">
              <h3 className="text-sm font-bold mb-3 text-[var(--primary)]">Prizes</h3>
              <div className="flex flex-wrap gap-2">
                {WHEEL_SEGMENTS.filter((s, i, arr) => arr.findIndex(x => x.label === s.label) === i).map((seg, i) => (
                  <div key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-[var(--card-border)]"
                    style={{ background: seg.color + '30' }}>
            ...
            {/* How it Works */}
            <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card)] p-4">
              <h3 className="text-sm font-bold mb-2 text-[var(--primary)]">How it Works</h3>
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-400">
                {['1 ticket = 1 spin', 'Click wheel or SPIN', 'Win → added to claimable SOL', 'CLAIM to withdraw'].map((s, i) => (
                  <span key={i} className="flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded-full shrink-0 flex items-center justify-center text-[9px] font-extrabold text-white"
                      style={{ background: 'linear-gradient(135deg, var(--primary), var(--accent))' }}>{i + 1}</span>
                    {s}
                  </span>
                ))}
              </div>
              <p className="text-[var(--primary)] font-semibold text-[10px] mt-2">Earn tickets from Daily Quests & Trading!</p>
            </div>
          </div>

          {/* RIGHT: Wheel */}
          <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card)] p-6 flex flex-col items-center justify-center">
